...
public class Calculator {
    ...
    public String evaluate(String str) {
        ...
        if (str.contains("(")) str = computeParenthesesPart(str); //parentheses first
        if (!confirmValidity(str)) return null;
        ...
        for (int i = 0; i < byPluses.length; i++) //calculating
        {
     ...
     * parentheses
     *
     * @param string with parentheses
     */
    private String computeParenthesesPart(String str) {
        int parenthesese = getTotalParenthesesPart(str);
        ...
        while (parenthesese > 0) {
            ...
            String result = computeSingleParenthesesPart(innMstPrtSubStr);
            if (result == null) str = str.replace(innMstPrtSubStr, "0");
            else str = str.replace(innMstPrtSubStr, result);

            parenthesese = getTotalParenthesesPart(str);
        }

        return str;
    }

    private int getTotalParenthesesPart(String str) {
        int counter = 0;
    ...
    private String computeSingleParenthesesPart(String str) {
        String result = "";
        if (str.indexOf("(") != -1) {
